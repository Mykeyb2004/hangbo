...
- 图表页标题与前一页数据页保持一致
- 左侧渲染图表
- 右侧渲染分析文字框，默认字号为 `18pt`
- 右侧分析文字框会优先按文本实际高度自适应外边框高度
- 若默认行距足够容纳文本，则保持默认行距，不因行数少而额外放大
- 若文本较长，则先向下扩展文字框高度；仅在接近页底安全边距时才继续压缩行距/段后距
- 二级指标数量为 2 时使用柱状图
...
当前默认会输出结构化短评，优先复用到备注页和图表页右侧文字框：

- `总体判断：...`
- `亮点：...`（仅当总体满意度达到阈值时才允许输出）
- `关注点：...`

补充约束：

- 必须直接使用客户群体名称，不使用“本页”“该页”等指代
- 只基于满意度和重要性均存在的有效行生成短评
- 默认控制在较短篇幅，适配图表页 `18pt` 可见文本显示
- 默认避免使用“拖累”“拖累项”，改用“弱势项”“相对偏弱”等中性管理表达
- 会在整批 PPT 内记录已使用的总体判断开头，并轮换表达风格，尽量减少跨页套话重复
...
- 若大模型接口在重试窗口内仍失败，该页会自动降级为占位备注并继续生成整份 PPT
- 若存在降级页面，会在正式 PPT 同目录额外写出 `*.llm_failures.log` 记录失败页清单，便于后续人工补写
